...
<body>
    ...
    <?php
           ...
           $sql = "SELECT product_image FROM product WHERE product_id = 127";
...
if (mysqli_num_rows($result) > 0) {
    ...
    // Retrieve the other product images
    $sql = "SELECT product_image FROM product WHERE product_id = 128"; 
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $small_images .= '<div class="small-img-col"><img src="' . $row["product_image"] . '" width="100%" class="small-img" alt=""></div>';
    ...
    // Retrieve the other product images
    $sql = "SELECT product_image FROM product WHERE product_id = 129"; 
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $small_images .= '<div class="small-img-col"><img src="' . $row["product_image"] . '" width="100%" class="small-img" alt=""></div>';
    ...
    // Retrieve the other product images
    $sql = "SELECT product_image FROM product WHERE product_id = 130"; 
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $small_images .= '<div class="small-img-col"><img src="' . $row["product_image"] . '" width="100%" class="small-img" alt=""></div>';
...
echo '
 ...
 <div class="single-pro-details">
        <?php
        include "./config/dbconnect.php";
        $sql = "SELECT product_name,product_brand,product_desc,price  FROM product WHERE product_id = 127";
        $result = mysqli_query($conn, $sql);
        $product_name = "";
        $product_brand = "";
        $product_desc = "";

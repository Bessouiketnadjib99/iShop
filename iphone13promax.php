...
<head>  
     ...
     <title> iShop-iPhone13Pro Max </title>
...
<body>
    ...
    <?php
           ...
           $sql = "SELECT product_image FROM product WHERE product_id = 95";
...
if (mysqli_num_rows($result) > 0) {
    ...
    // Retrieve the other product images
    $sql = "SELECT product_image FROM product WHERE product_id = 95"; 
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $small_images .= '<div class="small-img-col"><img src="' . $row["product_image"] . '" width="100%" class="small-img" alt=""></div>';
        }
    }
    
    // Retrieve the other product images
    $sql = "SELECT product_image FROM product WHERE product_id = 96"; 
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $small_images .= '<div class="small-img-col"><img src="' . $row["product_image"] . '" width="100%" class="small-img" alt=""></div>';
    ...
    // Retrieve the other product images
    $sql = "SELECT product_image FROM product WHERE product_id = 97"; 
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $small_images .= '<div class="small-img-col"><img src="' . $row["product_image"] . '" width="100%" class="small-img" alt=""></div>';
    ...
    // Retrieve the other product images
    $sql = "SELECT product_image FROM product WHERE product_id = 98"; 
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $small_images .= '<div class="small-img-col"><img src="' . $row["product_image"] . '" width="100%" class="small-img" alt=""></div>';
...
echo '
 ...
 <div class="single-pro-details">
        <?php
        include "./config/dbconnect.php";
        $sql = "SELECT product_name,product_brand,product_desc,price  FROM product WHERE product_id = 95";
        $result = mysqli_query($conn, $sql);
        $product_name = "";
        $product_brand = "";
        $product_desc = "";
    ...
    echo '<option value="256 Gb">256 Gb</option>';
    echo '</select>';
    echo '<select name="product_color">';
    echo '<option>Select Color</option>';
    echo '<option value="Blue">Blue</option>';
    echo '<option value="Graphite">Graphite</option>';
